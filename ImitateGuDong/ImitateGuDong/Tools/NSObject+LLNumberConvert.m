...
NSString *const LLNumberUnitList = @"| 十 | 百 | 千 | 万 | 十 | 百 | 千 ";

@implementation NSString (LLNumberConvert)
...
+ (NSString *)ll_timeStringWithTimeValue:(NSInteger)timeValue {
    ...
    if (h > 0) {
        [textM appendFormat:@"%@ 小时 ", [self ll_numberStringWithNumber:h hasDotNumber:NO]];
    }
    ...
    if (h > 0 && m == 0) {
        ...
        [textM appendFormat:@"%@ 分钟 ", [self ll_numberStringWithNumber:m hasDotNumber:NO]];
    }
    
    NSInteger s = timeValue % 60;
    [textM appendFormat:@"%@ 秒", [self ll_numberStringWithNumber:s hasDotNumber:NO]];
...
+ (NSString *)ll_numberStringWithNumber:(double)number hasDotNumber:(BOOL)hasDotNumber {
    ...
    // 1. 准备工作
    NSArray <NSString *>*unitArray = [LLNumberUnitList componentsSeparatedByString:@"|"];
